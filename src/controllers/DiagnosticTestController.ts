...
import { parsePagination, resolveTeacherId } from "../utils/helpers";
...
class DiagnosticTestController {  
  ...
  public getMyDiagnosticTestsAsTeacher = async (req: Request, res: Response): Promise<void> => {
    try {
      const teacherId = await resolveTeacherId(req, res);
      if (!teacherId) return;
      ...
      const result = await DiagnosticTestService.getDiagnosticTestsByTeacher(teacherId, page, limit, search);
      sendSuccess(res, 200, "Daftar tes diagnostik Anda berhasil didapatkan", result);
    } catch (error: any) {
      sendError(res, error.status || 500, error.message || "Gagal mendapatkan daftar tes diagnostik Anda");
    }
  }
